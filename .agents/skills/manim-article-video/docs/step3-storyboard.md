...
写 `shipinhao/storyboard.md`：段落表（场景 / 动画要点 / 配音稿），每段一个场景，**5-6 段**（2026-08-25 用户拍板：场景从 8 段收紧，控节奏）。配音稿是字幕基准，措辞须与文章术语、年份一致（2026 等，见 AGENTS.md 数据时效性）。

**台词爆点节奏规范（硬性，完播率抓手）**：

1. **开场钩子（S1 前 3 秒，硬性）**：第一句必须是价值承诺或悬念——「这条视频讲清楚 XXX 为什么 YYY」/「XXX 出了问题，问题出在 XXX」，禁止开场寒暄/背景铺垫（完播率低的主因是开头 3 秒流失）
2. **每段至少 1 个爆点**，5-6 段内 4 种爆点类型轮换、均匀分布——禁止爆点全挤开头、中段平铺直叙：
...
7. **结尾预告 + 互动（硬性）**：S5/S6 末尾预告下一篇内容（「下一篇拆 DualPipe」）+ 抛 1 个开放式问题引导评论区（「你觉得 1M 上下文真有必要吗？评论区聊聊」）
...
9. **动画降噪（2026-08-25 拍板）**：分镜里每页只标注 **1 个主视觉动效**（数字滚动/轨迹/拉幕选一），其余元素静态出现；camera_zoom/morph/trace_dot/breathe 每片 ≤3 处，只给爆点/公式；emphasize 每片 ≤5 次——动画密度超限的分镜在 Step 3 就砍掉，不带到 scenes.py
10. 参照：`content/2026-07-17-位置编码/shipinhao/storyboard.md`（分镜）、`.../full.txt`（配音稿含拟声标签与 `<#0.5#>` 用法）
