...
这件事情在 DeepSeek-R1 刚刚推出来的 2025 年初，可能还没有那么重要，然而背后的原因和副作用就很快引起了社区研究者们的关注，其实非常简单，是三段可以写在黑板上的数学谜题。组内没有差异时，信号消失；功劳平摊时，信号有毒；按长度归一化时，信号被带偏。2025 年，DAPO、Dr. GRPO、GSPO 三篇论文修的，恰好就是这三堵墙。
...
正确率 50% 的题，全同分几乎不可能（0.003%）；正确率 90%，概率 18.5%；95%，44.0%；99%——85.1%。模型越练越强，简单题成批退化成全对组。看起来，似乎是训练得越好，信号反而给的越少了？
...
好答案里面的浑招也会被抬高了，而一堆烂答案里面的妙手却被连坐拉低。
...
GRPO 的损失里，每份答卷还要按长度归一（除以回答长度）。负面答卷越长，每个 token 摊到的惩罚越薄。模型会无意间从这里学习到，给的答案越长越好。这也是很多人诟病的老版本的 DeepSeek 经常思考和输出过长的原因。
...
DAPO 补信号，Dr. GRPO 纠偏差，GSPO 稳噪声，但没有一家把 critic 请回来。这一步到底是值得还是不值得，其实我自己也思考了很久，觉得真正值得写的，还是底层的原因和机制，到底崩在哪个地方了。

大模型强化学习训练算法设计，有时候就是这样会陷入两难的境地，甚至有省下来的和失去的竟然是同一个东西的怪现象。你觉得在 agent 长程任务当道的今天，当年轰动一时的 GRPO 算法，是否已经是明日黄花了吗，还是会换个方式王者归来？说说你的看法。
