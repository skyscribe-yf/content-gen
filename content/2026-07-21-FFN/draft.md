...
关键不在于再开一次圆桌。注意力已经把其他 token 的相关线索汇进当前 token；FFN 不再开圆桌，却负责把这份**带上下文的表示**继续加工。

这也是最容易被误解的地方：**逐 token 独立计算，描述的是 FFN 这一步不再读取别的 token，不是说它没有上下文。** 上下文已经在前一步写进了 token 的向量。
...
这也澄清一个容易混淆的点：说 FFN “独立”并不是说它不知道上下文。xᵢ 早已在上一小步的注意力中混入了上下文；FFN 只是不会在自己的计算里再开一次圆桌。
...
第一步和第二步都把 d_model 维的输入送往更宽的 d_ff 维空间。可以把它理解成：对这份已经带上下文的表示，先临时展开出更多可组合的加工方向。这里的“更丰富”是容量上的描述，并不意味着某一维必然对应某个可命名的语义。
...
批量计算得到的 y 的两行，和把每一行单独送进同一组权重后得到的结果完全相等。更关键的是，两次扰动都通过了：改动第 1 个 token，第 2 个输出逐元素完全不变；反过来也一样。

这些固定数值只为让计算可复现。真正有力的检查是“扰动一个输入，观察另一个输出”：在这个 FFN 内，没有任何跨行运算把变化传过去。

结论的边界也要画清：它只证明这个 FFN 层不混合 token，不是说 token 脱离了上下文。前面的注意力会让 xᵢ 本身随上下文改变；后一层注意力也仍可能把信息再次传开。

![](03-fixed-matrix-experiment.png)

## 🌍 真实国产模型的一条旁注

教学用的 2 维矩阵很小，真实模型当然大得多。以 2026 年 [DeepSeek-V4-Pro 固定版本的 `config.json`](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/raw/b5968e9190ef611bbf34a7229255be88a0e937c1/config.json) 为例，配置写有 `hidden_size: 7168` 与 `hidden_act: "silu"`：前者是 token 主表示的宽度，后者是激活函数设置。它们说明真实工程同样要决定表示宽度与门控中的非线性；至于专家怎样路由，要看下一节。配置是声明，不是运行轨迹，更不能单独推出能力因果。
...
所以先把 dense FFN 的逐 token 变换看懂，读 MoE 才不会把“选专家”误认为“取消 FFN”。想看真实国产模型怎样用路由把多个 FFN 组织起来，可回读[《DeepSeek便宜30倍的秘密：MoE混合专家入门》](https://mp.weixin.qq.com/s/QdkD0CR2fD-HfY77-gX3Ug)。
...
回头看整层的节奏，就是：**圆桌交流 → 独立思考 → 保留原话，并整理尺度。** 注意力先把相关上下文写进每个 token，FFN 再独立加工这份带上下文的表示；残差连接和归一化则把结果安全接回整层。注意力没有让 FFN 多余，反而让两者的分工更清楚。
