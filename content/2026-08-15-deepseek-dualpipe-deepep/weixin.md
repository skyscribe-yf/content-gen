...
cover: "00-cover.png"
wechatUrl: "https://mp.weixin.qq.com/s/8WS2AjSkac7CstjToJclxA"
...
**等待 2：等网络。** MoE 层里，每个 token 要被 dispatch 到持有对应专家的 GPU 上算，算完 combine 回来。上一篇文章算过这笔账（[五维并行篇](https://mp.weixin.qq.com/s/ae1iwvau14gFCnyfg4hVIw)），EP 每层要传约 **16GB**——FP8 dispatch 加 BF16 combine。跨节点的 InfiniBand 带宽只有 NVLink 的约 1/18——900 GB/s 对 50 GB/s。这 16GB 是实打实的等待。
...
$F\&B$ 是「前向/反向重叠块」的耗时，$W$ 是反向中算权重的那部分。PP=16 时，块数从 15 降到 7，气泡直接减半以上。实验脚本跑了一下（演示值，见文末）：PP=16 时气泡从 45 降到 12.6，**不到原来的 1/3**。
...
**真货 1：专用内核。** 通用 NCCL 的 all-to-all 要为任意数据形状兜底，dispatch 的形状却是固定的——token 数 × top-k × hidden 维。专用内核把通用性省下的开销，全换成带宽。dispatch 和 combine 各自还有高吞吐、低延迟两条路径：大批量要吞吐，per-token 级同步要低延迟。两个场景，两套内核。FP8 dispatch 直接把带宽砍半（低精度省带宽的原理，[FP8 篇](https://mp.weixin.qq.com/s/yxrkmxPSZ8CnsFhWZ1bCPA)讲过）。内核是 JIT 编译的——第一次用要先现场编译，慢得想退货，编完就快了。SM100 上 NVLink 实测能到 726/740 GB/s。
...
所以单对通信在节点内能藏满，跨节点藏不满。V3/V4 的所有机制，本质都是同一件事：把一次通信摊到一大批计算上。DualPipe 跨 micro-batch 摊，MegaMoE 跨 wave 摊。摊平之后，有效 Vcomp/Vcomm 才抬得过 6144 的门槛。下一节就是这套思路的极限形态。

![C/B 判据：算力与带宽的天平](06-cb-chart.png)

## 五、甜点：MegaMoE 把藏做到 wave 粒度

[五维并行篇](https://mp.weixin.qq.com/s/ae1iwvau14gFCnyfg4hVIw) 上篇只给了 MegaMoE 的加速数字，今天看机制。
...
用上一节的判据说：单个 token-expert 对的通信跨节点藏不满。但把一整批 wave 的计算量摊给一次通信，有效 Vcomp/Vcomm 就被抬过 6144 的门槛。wave 越深，摊得越平：batch 越小、通信占比越大，摊平的收益越明显。

理论加速 1.92×（V4 报告 Figure 5），实测 1.50~1.73×。RL/agent 这种长尾小 batch 场景反而最吃这套，实测 1.96×。实现开源在 DeepGEMM 里，名字叫 **MegaMoE2**。

顺带一个发现：V4 报告全文一次都没提 DeepEP。不是不用了——是进化了：DeepEP 解决「层内怎么藏」，MegaMoE 解决「怎么藏得更满」，同一套哲学的两代答案。
...
至此训练系统线收官：[五维并行篇](https://mp.weixin.qq.com/s/ae1iwvau14gFCnyfg4hVIw) 讲怎么切（层、矩阵、批次、专家、序列），[上下文并行篇](https://mp.weixin.qq.com/s/UB_ILj-62K3VBUY4_AopSw) 讲怎么切序列，本篇讲怎么藏等待。切得开、装得下、等得起——大模型训练系统这三个坎，DeepSeek 用 V3 的工程积累和 V4 的进化解完了。
