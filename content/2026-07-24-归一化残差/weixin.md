...
前一篇 [前馈网络 FFN](https://mp.weixin.qq.com/s/vBCzukDlQyB9O6ASgAmlvQ) 讲完了 Attention 之后，每个 token 独立做非线性变换的那一步。但你会发现：不管是 Attention 还是 FFN，正式计算之前都先过了一次归一化。

[残差连接](https://mp.weixin.qq.com/s/xefNN9Gjaw3TKl60KeHzAg) 解决的是一件很重要的事：深度神经网络里，原始信号不必每次都穿过一个复杂变换，可以沿捷径直接往下走。
...
值得一提的是，DeepSeek-V4-Pro 的残差连接没有停在最朴素的 $x + F(x)$。同一份配置里的 `hc_mult: 4` 和代码中的 Hyper-Connections，把残差的"直通车"升级成了可学习的"多车道"——它维护并混合 4 份隐藏状态，替代单一捷径。

归一化依然守在每条车道的入口——Attn 和 FFN 前面那两次 RMSNorm——稳住了子层输入尺度。残差本身怎么连线、怎么进化，是另一条独立的工程方向。

还记得[前一篇残差连接](https://mp.weixin.qq.com/s/xefNN9Gjaw3TKl60KeHzAg)里那张"进化路线"表吗？从加法捷径到 Highway Network 的加权混合，残差一直在进化。Hyper-Connections 是这条路线的最新一站——但今天先不展开，留到后面专门聊。
...
残差连接保住 $x$ 的直通路，却不保证 $x + F(x)$ 的整体范数不变；Pre-Norm 用 LayerNorm 或 RMSNorm 先稳定子层输入，再把增量加回去。这样深 Transformer 才能同时拥有可控尺度和可走通的梯度路径。

你会更愿意把一条残差主路保持为完全不动的"原件通道"，还是让它也学会混合多份状态？哪一种更让你安心？评论区聊聊。

前一篇 [前馈网络 FFN](https://mp.weixin.qq.com/s/vBCzukDlQyB9O6ASgAmlvQ) 讲了 Attention 把上下文汇入每个 token 后，FFN 如何独立提炼。下一篇回到 Transformer 全景：跟一句话走完从输入到预测的完整流程，把 Attention、FFN、残差和归一化串成一条闭合回路。

关注「数解AI」，把 Transformer 拆成一条能自己推演的链。

📖 **[大模型原理合集](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=MzkyMzQyODExNQ==&action=getalbum&album_id=4597831652025925632#wechat_redirect)**：① [BPE分词：AI为什么把文字切成碎片？](https://mp.weixin.qq.com/s/5nR_KI47v_U8KwpQA4Uv5Q) → ② [词嵌入为什么让AI懂"苹果"？5万个0变坐标](https://mp.weixin.qq.com/s/rDryn1z_hLt7mwi3X8fsxQ) → ③ [位置编码怎么工作？词序一错意思全变](https://mp.weixin.qq.com/s/4nO2VqQLaYxGdDmtQeypCQ) → ④ [注意力机制是什么？别再当数据库查询](https://mp.weixin.qq.com/s/KrilwX6VRjI9KfjvD7C6kw) → ⑤ [Attention都够了，为什么还要前馈网络？](https://mp.weixin.qq.com/s/vBCzukDlQyB9O6ASgAmlvQ) → ⑥ 归一化为什么总在前面？61层模型靠它不崩（本篇） → ⑦ 注意力找人，FFN存知识：跟一句话走完Transformer（待发布）
...
#归一化 #RMSNorm #Pre-Norm #大模型原理 #数解AI
