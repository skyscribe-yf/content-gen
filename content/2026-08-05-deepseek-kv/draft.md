...
上一篇再接着看 CSA/HCA：注意力究竟要看多少？现在再往下追，DeepSeek-V4 里还有一个更反直觉的设计。
模型把每个块的 KV 压成一个 entry，再让这个 entry 同时当 key 和 value。
...
CSA 怎么把一份 entry 压出来，上一期已经推过，这里只留关键两步。
...
而这套注意力还要和 V4-Pro 的 61 层网络一起工作，这就引出下一篇：**mHC（流形约束超连接）**。下一篇拆它怎么让深层信号稳定传播——为什么普通残差连接在 60 层以上会「传话传没」。

📖 **[大模型原理合集](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=MzkyMzQyODExNQ==&action=getalbum&album_id=4597831652025925632#wechat_redirect)**：BPE → 词嵌入 → 位置编码 → 注意力 → KDA 长上下文 → MLA → FFN → 归一化残差 → Transformer 全景 → 预训练 → Kimi K3 架构 → SFT → RLHF → PPO → GRPO → RLVR → 推理加速

🔥 **DeepSeek 技术解密**：AI 上下文为什么越长越慢 → MoE 混合专家 → MLA → V4 注意力 → **K=V（本篇）** → mHC → FP8 训练
