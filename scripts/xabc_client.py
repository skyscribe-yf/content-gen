"""
xabcimg.com 图片生成 API 客户端

用法:
  # 单张生成
  XABC_MING_SESSION=xxx python xabc_client.py \
    --prompt "小红书风格知识科普卡片..." \
    --size 1024x1536 --quality high --n 1

  # 批量系列图（从 JSON 配置）
  XABC_MING_SESSION=xxx python xabc_client.py \
    --config ../content/example-series-xabc.json

  # 查询余额
  XABC_MING_SESSION=xxx python xabc_client.py --balance

环境变量:
  XABC_MING_SESSION  — 必填，xabcimg.com 登录 session cookie
"""
import argparse
import base64
import json
...
def _session() -> str:
    ...
    if not s:
        sys.exit("❌ 请设置环境变量 XABC_MING_SESSION")
    return s
...
def _api(path: str, method: str = "GET", **kwargs) -> dict:
    """调用 xabcimg API"""
    url = f"{BASE_URL}{path}"
    cookies = {"xabcming_session": _session()}
...
def submit_generate(
    ...
    return _api("/api/images", method="POST", data=data, timeout=120)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", default="1024x1536", help="尺寸 (默认 1024x1536 即3:4竖版)")
